...
TEST_CASE("GodviewRecording finds target or combat event snapshots per map", "[godview][recording]") {
    ...
    auto firstTargetMap0 = recording.findEventMs(999.0, 0, GodviewRecording::EventKind::Target, 1);
    REQUIRE(firstTargetMap0);
    REQUIRE(*firstTargetMap0 == 1000);
    REQUIRE_FALSE(recording.findEventMs(1000.0, 0, GodviewRecording::EventKind::Target, 1));
